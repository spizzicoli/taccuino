...
async function checkAIStatus(){
  // L'assistente AI richiede una funzione lato Supabase non ancora collegata in questa
  // versione ibrida: per ora resta sempre disponibile=false, senza tentare chiamate a vuoto.
  aiAvailable = false;
}
...
function downloadBackup(){
  ...
  const a = document.createElement('a'); a.href=url; a.download=`backup-taccuino-${todayStr()}.json`;
...
function importBackup(input){
  ...
  reader.onload = (e) => {
    try{ state = Object.assign(state, JSON.parse(e.target.result)); saveState(); toast('Backup importato.'); }
...
function restoreFromTrash(trashId){
  ...
  switch(t.type){
    ...
    case 'homeTask': state.homeTasks.push(t.data); upsertExpense('task-'+t.data.id, t.data.title, t.data.cost, todayStr(), 'Lavori casa'); break;
...
// ---------- MODAL ----------
function openModal(html){ document.getElementById('modalBody').innerHTML = html; document.getElementById('overlay').classList.add('active'); }
  ...
  app.innerHTML = `
    <div class="header">
      ...
      <div class="header-right">
        ...
        <button class="btn icon-btn" title="Briefing di oggi" onclick="openBriefingModal()">${ICONS.wand}</button>
        ...
        <label class="btn" style="display:inline-flex;align-items:center;">Importa<input type="file" accept=".json" style="display:none" onchange="importBackup(this)"></label>
        <button class="btn subtle" onclick="downloadBackup()">Scarica copia</button>
      </div>
    </div>
    <nav class="tabs">
...
function renderHome(){
  ...
  <div class="card">
    <div class="card-head"><div class="title-group">${sectionIcon('spark','var(--c-casa)','var(--c-casa-soft)')}<h2 style="font-size:16px;">Chiedi al tuo taccuino</h2></div></div>
    ${aiAvailable? `
      <div class="section-label">Fai una domanda sui tuoi dati, es. "Quanto ho speso in bollette questo mese?" oppure "Quando scade l'assicurazione dell'auto?".</div>
      <div style="display:flex;gap:8px;">
        <input id="aiAskInput" placeholder="Fai una domanda…" style="flex:1;padding:10px 12px;border:1px solid var(--line);border-radius:10px;" onkeyup="if(event.key==='Enter') askTaccuino()">
        <button class="btn primary" onclick="askTaccuino()">Chiedi</button>
      </div>
      <div id="aiAskAnswer"></div>
    ` : `<div class="notice">L'assistente AI non è ancora attivo in questa versione ibrida (richiede una funzione lato Supabase, arriverà con un prossimo aggiornamento).</div>`}
  </div>
  <div class="stat-row">
...
async function askTaccuino(){
  ...
  try{
    ...
    const d = await r.json();
    if(answerBox){
      answerBox.innerHTML = d.ok
        ? `<div class="item" style="margin-top:10px;background:var(--paper);"><div class="item-desc">${esc(d.answer)}</div></div>`
        : `<div class="notice" style="margin-top:10px;">Non sono riuscito a rispondere: ${esc(d.reason||'errore sconosciuto')}.</div>`;
    }
  }catch(e){
    if(answerBox) answerBox.innerHTML = `<div class="notice" style="margin-top:10px;">Errore di connessione al server.</div>`;
  }
}
...
function renderHouse(){
  ...
  <div class="card">
    ...
    ${bills.length? bills.map(b=>{
      ...
      return `
      <div class="item"><div class="item-top">
        <div><div class="item-title">${esc(b.provider)} ${anomaly?`<span class="tag ${anomaly.good?'badge-good':'badge-anomaly'}">${anomaly.text}</span>`:''}</div>
...
function openBillForm(id){
  ...
  openModal(`
    ...
    <div class="field"><label>Metodo di pagamento</label><input id="f_method" value="${esc(b.method||'')}" placeholder="es. addebito diretto, bonifico"></div>
...
function markBillPaid(id){
  ...
  addExpense('bill-payment-'+uid(), 'Pagamento '+b.provider, b.amount, todayStr(), 'Bollette');
  ...
  saveState(); toast('Pagamento registrato nel salvadanaio.');
}
...
function saveTask(id){
  ...
  upsertExpense('task-'+item.id, item.title, item.cost, todayStr(), 'Lavori casa');
...
function renderSidebar(){
  ...
  document.getElementById('sidebar').innerHTML = `
    ...
    <button class="btn primary" style="width:100%;margin-bottom:14px;" onclick="openEventForm(null,'${ds}')">+ Aggiungi impegno</button>
    ${evts.length? evts.map(e=>`
      <div class="item">
        <div class="item-top">
          ...
          <div class="item-actions"><button class="btn small ghost" onclick="openEventForm('${e.id}','${ds}')">Modifica</button><button class="btn small danger ghost" onclick="confirmDelete('Eliminare questo impegno?', ()=>deleteEvent('${e.id}'))">Elimina</button></div>
        </div>
...
function renderSettings(){
  ...
  <div class="card">
    <div class="card-head"><h2 style="font-size:16px;">Copia di backup</h2></div>
    <div class="section-label">Anche con i dati sul cloud, puoi scaricare una copia in qualsiasi momento dal pulsante "Scarica copia" in alto — buona abitudine prima di grandi modifiche.</div>
  </div>
  <div class="card">
